...
% note that here, the vector zc refers to all (not only internal) 
% metabolites, but the entries for external metabolites are ignored

if length(cba_constraints_act.zc),
  zc_act = cba_constraints_act.zc(ind_met_active);
else,
  zc_act = zeros(size(c_act));
end

Q_act_predefined = - c_act .* zc_act;
dum              = nan * ones(nm,1);
...
% zc_int_act_updated = -Q_act(ind_int_act)./c_act(ind_int_act);
...
zc_updated = - Q ./ c;
zc_updated(find(network.external)) = 0;
zc_updated(isnan(zc_updated)) = 0;

Rfvu = [ cba_constraints.zv' * R.RJ ]';
Rfcu = [         zc_updated' * R.RS ]';
Rfu  = Rfvu + Rfcu;
